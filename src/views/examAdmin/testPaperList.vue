  ...
  <div>
    ...
    >
      <div slot="formContent">
        ...
        <!-- <el-transfer filterable v-model="yesData" :props="{key: 'id',label: 'questionContent'}" :titles="['题海', '选中的题目']" @change="handleChange" :data="noData"
    ></el-transfer>
         -->


<el-transfer filterable v-model="yesData" :data="noData" :titles="['题海', '选中的题目']" @change="handleChange">
	<p slot-scope="{ option }">
    <span>{{ option.label }}</span>
		<span class="ml-15">（{{ option.questionBonus }}分）</span>
	</p>
</el-transfer>
    ...
    <el-dialog
      ...
      <ul class="testContent">
        <li v-for="(item,index) of testContent" :key="index">
          ...
          <p v-if="item.tQuestionPack.answerA"><b>A：{{item.tQuestionPack.answerA}}</b>
            <img v-if="item.tQuestionPack.aFileName" :src=baseURL+item.tQuestionPack.aFileName /> 
          </p>
          <p v-if="item.tQuestionPack.answerB"><b>B：{{item.tQuestionPack.answerB}}</b>
            <img v-if="item.tQuestionPack.bFileName" :src=baseURL+item.tQuestionPack.bFileName /> 
          </p>
          <p v-if="item.tQuestionPack.answerC"><b>C：{{item.tQuestionPack.answerC}}</b>
            <img v-if="item.tQuestionPack.cFileName" :src=baseURL+item.tQuestionPack.cFileName /> 
          </p>
          <p v-if="item.tQuestionPack.answerD"><b>D：{{item.tQuestionPack.answerD}}</b>
            <img v-if="item.tQuestionPack.dFileName" :src=baseURL+item.tQuestionPack.dFileName /> 
          </p>
          <p v-if="item.tQuestionPack.answerE"><b>E：{{item.tQuestionPack.answerE}}</b>
            <img v-if="item.tQuestionPack.eFileName" :src=baseURL+item.tQuestionPack.eFileName /> 
          </p>
          <p v-if="item.tQuestionPack.answerF"><b>F：{{item.tQuestionPack.answerF}}</b>
            <img v-if="item.tQuestionPack.fFileName" :src=baseURL+item.tQuestionPack.fFileName /> 
          </p>
      ...
      try {
        ...
        if (data.status == 0) {
           ...
           for(let i = 0; i < allData.length; i++) {
              dataArr.push({
                key: allData[i].id,
                label: allData[i].questionContent,
                questionBonus: allData[i].questionBonus,
              });
            }
            this.noData = dataArr;
